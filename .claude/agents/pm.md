...
- Size the spec by cohesion: one task is a chunk an implementer can carry to a coherent, finished state and a human can review in one sitting. A single user-visible surface — a page, a flow, a resource end-to-end — is one task. Decompose for a hard dependency, a risky unknown worth landing alone, or a diff too large to review honestly; "it could be smaller" doesn't qualify. When decomposition is warranted it is your job, not the implementer's: spec the first task, file the rest as new backlog items.
