...
anything. Review layers on top of deterministic checks, never replaces them, and one
runner owns each gate rather than every agent repeating it.
...
- Verification is executed, not argued: any claim about behavior is backed by a command that actually ran (a test, a smoke check, a reproduced output). Run the narrowest check that could fail on what you just touched; the full contract runs once, at the boundary.
- Tasks are sized by cohesion: one task is what can be finished coherently and reviewed in one sitting. A user-visible surface — a page, a flow, a design system — is one such unit, since splitting one is how interfaces end up looking assembled rather than designed.
- Committed generated files are regenerated, never hand-edited. Run the generator, commit its output. The `gates` job fails when running the contract changes tracked files.
